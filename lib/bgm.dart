...
class BGM {
  ...
  static Future<void> _update() async {
    ...
    if (isPaused) {
      ...
      if (current == BGMType.playing) await playing.fixedPlayer.pause();
    } else {
      if (current == BGMType.home) await home.fixedPlayer.resume();
      if (current == BGMType.playing) await playing.fixedPlayer.resume();
    }
  }
  ...
  static Future<void> play(BGMType what) async {
    if (current != what) {
      ...
      } else if (what == BGMType.playing) {
        await home.fixedPlayer.stop();
        await playing.loop("bgm/playing.mp3", volume: .25);
      }
